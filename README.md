...
일본어 만화 이미지를 한국어로 번역하고, 원문 지우기와 보정, PNG 출력까지 한 앱에서 처리하는 **Windows 데스크톱 만화 번역 편집기**입니다.

초기 버전은 번역 오버레이로 빠르게 읽는 데 집중했지만, 현재는 번역 블록 편집, Flux 인페인팅, 수동 보정, 색상/폰트 조정, 결과 PNG 출력까지 포함합니다. 개인 감상용 번역본을 빠르게 만들고 다듬는 흐름에 맞춰져 있습니다.
...
6. 번역 결과가 어색하면 텍스트 블록, 문장, 폰트, 색상, 위치를 수정합니다.
7. 필요하면 `인페인팅` 모드에서 원문을 지우고 남은 자국을 보정합니다.
8. 완성된 페이지는 `PNG 출력`으로 내보냅니다.
...
- 기본 Gemma 4 31B 프리셋은 스모크 테스트 기준으로 GPU 전체 로드를 사용하고, VRAM 절약은 `VRAM 모드`에서만 조절합니다.
...
`인페인팅` 모드는 원문 글자와 주변 배경을 정리한 뒤 번역 블록을 얹어 완성 PNG를 만들 때 사용합니다.

- `자동`: 현재 페이지 또는 남은 페이지의 원문 배경을 Flux 인페인팅으로 지웁니다.
- `보정`: 마스크 붓으로 효과음이나 남은 자국을 칠해 다시 지우고, 일반 붓/복원/색 뽑기로 자잘한 부분을 다듬습니다.
- `원본 비교`: 인페인팅 전 원본과 결과를 빠르게 비교합니다.
- `출력`: 인페인팅 결과 위에 번역 블록을 렌더링한 PNG 파일을 만듭니다.

인페인팅 결과는 원본 이미지를 덮어쓰지 않고 별도 파일로 저장됩니다. 마음에 들지 않으면 페이지 단위 또는 화 전체를 원본으로 되돌릴 수 있습니다.
...
이 앱은 개인 감상용 번역과 편집을 빠르게 끝내는 데 초점을 둡니다. 전문 출판/상업 제작 도구처럼 모든 식자 작업을 세밀하게 관리하는 것을 목표로 하지는 않습니다.

- 자동 번역, OCR, 인페인팅 결과는 페이지마다 품질 차이가 있을 수 있습니다.
- 복잡한 배경, 큰 효과음, 손글씨, 톤 위 글자는 수동 보정이 필요할 수 있습니다.
- 현재 결과 출력은 PNG 중심입니다. PDF 제작이나 여러 레이어를 따로 관리하는 전문 DTP 기능은 포함하지 않습니다.
- 공유 파일은 앱에서 다시 열기 위한 데이터 패키지입니다. 완성 이미지 배포용 패키지와는 다릅니다.
